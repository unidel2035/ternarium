...
- [ ] Скачать коннектом дрозофилы (FlyWire, открытые данные) — таблица ~50 млн синапсов
- [ ] Конвертер: синапсы → разрежённая троичная матрица {-1, 0, +1} по типу нейромедиатора и порогу (~12 МБ)
- [ ] CPU-референс: numpy/sparse, полный проход + корректность
- [ ] Экспорт в memory-image для `tritmsparse`/`bitnet_layer`
