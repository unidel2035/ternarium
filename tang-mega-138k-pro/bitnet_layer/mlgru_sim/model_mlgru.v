...
module model_mlgru(input wire clk, input wire ce, input wire rst, input wire start,
  ...
  reg signed [15:0] lutrom[0:8*LN-1];
  ...
  function integer bWf;  input integer l; bWf =(l==0)?B_Wf0 :B_Wf1;  endfunction
  function integer bWc;  input integer l; bWc =(l==0)?B_Wc0 :B_Wc1;  endfunction
  function integer bWg;  input integer l; bWg =(l==0)?B_Wg0 :B_Wg1;  endfunction
  function integer bWo;  input integer l; bWo =(l==0)?B_Wo0 :B_Wo1;  endfunction
  function integer bW2g; input integer l; bW2g=(l==0)?B_W2g0:B_W2g1; endfunction
  function integer bW2u; input integer l; bW2u=(l==0)?B_W2u0:B_W2u1; endfunction
  function integer bW2d; input integer l; bW2d=(l==0)?B_W2d0:B_W2d1; endfunction
  function integer blf;  input integer l; blf =(l==0)?BL_Lf0 :BL_Lf1;  endfunction
  function integer blc;  input integer l; blc =(l==0)?BL_Lc0 :BL_Lc1;  endfunction
  function integer blg;  input integer l; blg =(l==0)?BL_Lg0 :BL_Lg1;  endfunction
  function integer bl2g; input integer l; bl2g=(l==0)?BL_L2g0:BL_L2g1; endfunction
  function integer mwo;  input integer l; mwo=(l==0)?MWo_0:MWo_1; endfunction
  function integer mwu;  input integer l; mwu=(l==0)?MWu_0:MWu_1; endfunction
  function integer mwd;  input integer l; mwd=(l==0)?MWd_0:MWd_1; endfunction
  // FSM
  ...
  always @(posedge clk) begin
    if(rst) begin stt<=IDLE; done<=0; g<=0; t<=0; l<=0; iq_st<=0; dv_st<=0; end
      ...
      RMS:  begin ss=0; for(d=0;d<D;d=d+1) ss=ss+mul16(x[d],x[d]); ms=ss>>>DSHIFT;
        iq_x<=ms*AS*AS; iq_st<=1; ret<=GSET; gph<=0; stt<=SQW; end
      FRMS: begin ss=0; for(d=0;d<D;d=d+1) ss=ss+mul16(x[d],x[d]); ms=ss>>>DSHIFT;
        iq_x<=ms*AS*AS; iq_st<=1; ret<=FSET; stt<=SQW; end
      LOG:  begin ss=0; for(d=0;d<D;d=d+1) ss=ss+mul16(x[d],x[d]); ms=ss>>>DSHIFT;
      ...
      NEXTL: begin if(l==L-1) begin if(t==S-1) stt<=LOG; else begin t<=t+1; stt<=LD; end end
