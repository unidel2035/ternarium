#!/usr/bin/env python3
"""export_ru_rtl.py — байтовая русская модель → RTL (hex+vectors) ДЛЯ model_mlgru.v.
Повторяет fixed-point семантику train_mlgru.py (та же, что в RTL). Сначала ПРОВЕРКА: генерит
NGEN байт русского в fixed-point из затравки «контакт: ». Если связно — экспорт. Запуск: python3 export_ru_rtl.py 24"""
import torch, torch.nn as nn, torch.nn.functional as F, numpy as np, math, sys, os
HERE=os.path.dirname(os.path.abspath(__file__))
D = int(sys.argv[1]) if len(sys.argv)>1 else 24
V,L,S = 256,3,48; H=2*D
PROMPT="контакт: "; NGEN=64

# ── архитектура (как в train_mlgru_ru.py) ──
...
m=Net(); m.load_state_dict(torch.load(f"{HERE}/mlgru_ru_d{D}.pt")); m.eval()
print(f"загружен mlgru_ru_d{D}.pt | D={D} L={L} V={V} H={H}")

# ── fixed-point (КОПИЯ train_mlgru.py — та же семантика, что RTL) ──
...
# ── затравка байтами (CP1251), паддинг пробелами слева до S ──
...
def fx_gen(SHIN,n=NGEN):
    ...
    return out
# ── свип SHIN: выбираем лучший по «похоже на русский» (мало replacement-байт) ──
print("=== ПРОВЕРКА fixed-point генерации (предохранитель) ===")
best=None
for SHIN in (3,4,5,6):
...
print(f"\n=== ЛУЧШИЙ SHIN={SHIN} ===\n{txt!r}")
GOLD=fx_gen(SHIN)
GATE_OK = txt.count("�")<3 and ("действие" in txt or "контакт" in txt or "статус" in txt)
print(f"\nПРЕДОХРАНИТЕЛЬ: {'✅ связный русский → можно в RTL' if GATE_OK else '❌ fixed-point рассыпался → откат на доменную модель'}")
if not GATE_OK:
    print("STOP — не экспортирую (RTL врал бы)."); sys.exit(1)

# ── ЭКСПОРТ hex+vectors (формат model_mlgru.v) ──
RW=max(D,H)
MATS=[]
for l in range(L):
...
open(f"{HERE}/mlgru_wt.hex","w").write("\n".join(rows_hex)+"\n")
open(f"{HERE}/mlgru_emb.hex","w").write("\n".join(f"{int(embq[r][c])&0xffff:04x}" for r in range(V) for c in range(D))+"\n")
LUTMATS=[]
for l in range(L):
...
open(f"{HERE}/mlgru_lut.hex","w").write("\n".join(lut_hex)+"\n")
MWF=12
def mwfp(mw): return int(round(mw*(1<<MWF)))
SEED=([0x20]*(S-len(pb))+pb)[-S:]
with open(f"{HERE}/mlgru_vectors.vh","w") as f:
    f.write("// БАЙТОВАЯ РУССКАЯ MLGRU (CP1251) — экспорт для RTL + tritterm\n")
...
print(f"экспорт: mlgru_wt.hex ({addr} строк), mlgru_emb.hex, mlgru_lut.hex ({li}), mlgru_vectors.vh | NGEN={NGEN}")
