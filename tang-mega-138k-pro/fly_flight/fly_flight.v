...
`include "fly_params.vh"
    ...
    reg [26:0] idle_cnt = 0;
    reg [15:0] stim_idx = 0;
    reg [23:0] wave_cnt = 0;                                  // радар-волна
    reg [11:0] stim_hist0 = 12'h092, stim_hist1 = 12'h096, stim_hist2 = 12'h05E;
    ...
    always @(posedge clk25 or negedge rst_n) begin
        if (!rst_n) begin
            ...
            wave_cnt <= 0; stim_hist0 <= 12'h092; stim_hist1 <= 12'h096;
            stim_hist2 <= 12'h05E;
        end else begin
            tx_start <= 0;
            pace_fire <= 0;
            ...
            case (mst)
                ...
                4'd8: begin                                         // M_TH1: порог + продвижение
                    ...
                    if (r == ROWS) begin
                        ...
                        // авто-стимул убран: в полётном режиме тычки идут по UART
                    end else begin
                        r   <= r + 1;
                        mst <= 4'd7;
                    end
    ...
    // ── ЭФФЕКТЫ (каждый завязан на реальный сигнал мозга) ──────────────
    wire [11:0] stim_pos = stim_id(stim_idx[1:0]);
    wire [7:0]  wave_r   = wave_cnt[23:16];            // радиус волны, период ~0.67 c
    wire [11:0] dist     = (tape_idx >= stim_pos) ? tape_idx - stim_pos
                                                 : stim_pos - tape_idx;
    wire        in_stim  = in_tape && (tape_idx == 12'hFFF);   // выключен: маркер только в демо
    wire in_wave  = in_tape && (dist == {4'd0, wave_r}) && (wave_r != 8'd0);
    wire in_glow0 = in_tape && (tape_idx == stim_hist0) && (tape_idx != stim_pos);
    wire in_glow1 = in_tape && (tape_idx == stim_hist1) && (tape_idx != stim_pos);
    wire in_glow2 = in_tape && (tape_idx == stim_hist2) && (tape_idx != stim_pos);
    // прогресс серии: 16 сегментов по 50 px, яркое = текущий шаг
    wire [15:0] stepm1  = step - 16'd1;
    wire [9:0]  segx    = px[9:0];
    wire in_prog_past   = (step != 16'd0) && (py >= 16'd40) && (py < 16'd58) && (segx < stepm1[9:0] * 10'd50);
    wire in_prog_cur    = (step != 16'd0) && (py >= 16'd40) && (py < 16'd58) && (segx >= stepm1[9:0] * 10'd50)
                        && (segx < step[9:0] * 10'd50);
    // рамка экрана цветом распознанного класса
    wire in_border = (px < 16'd4) || (px >= 16'd796) || (py < 16'd4) || (py >= 16'd476);
    ...
    always @(*) begin
        if (!visible)                 {r6, g6, b6} = 18'h00040A;
        else if (in_border) begin                                  // класс на рамке
            case (cls_reg)
                2'd0: {r6, g6, b6} = 18'h003F18;
                2'd1: {r6, g6, b6} = 18'h3F0808;
                2'd2: {r6, g6, b6} = 18'h083F3F;
                default: {r6, g6, b6} = 18'h3F3F08;
            endcase
        end
        else if (in_prog_past)        {r6, g6, b6} = 18'h002A2A;   // пройденные шаги
        else if (in_prog_cur)         {r6, g6, b6} = hb[22] ? 18'h3F3F3F : 18'h001818;
        else if (in_sweep)            {r6, g6, b6} = 18'h3F3F3F;
        else if (in_stim)             {r6, g6, b6} = 18'h3F3F3F;   // сенсор
        else if (in_wave)             {r6, g6, b6} = 18'h002A14;   // радар-волна
        else if (in_glow0)            {r6, g6, b6} = 18'h001E00;   // шлейф стимула
        else if (in_glow1)            {r6, g6, b6} = 18'h001200;
        else if (in_glow2)            {r6, g6, b6} = 18'h000A00;
        else if (in_cl) begin                                      // квадранты, пульс
            case (cls_q)
                2'd0: {r6, g6, b6} = cls_hit ? (hb[22] ? 18'h003F18 : 18'h001C0C) : 18'h000808;
                2'd1: {r6, g6, b6} = cls_hit ? (hb[22] ? 18'h3F0808 : 18'h1C0404) : 18'h080000;
                2'd2: {r6, g6, b6} = cls_hit ? (hb[22] ? 18'h083F3F : 18'h041C1C) : 18'h000808;
                default: {r6, g6, b6} = cls_hit ? (hb[22] ? 18'h3F3F08 : 18'h1C1C04) : 18'h080800;
            endcase
        end
            ...
            case (xa[tape_neur])
                ...
                default: {r6, g6, b6} = 18'h001028; // покой
            endcase
        end
